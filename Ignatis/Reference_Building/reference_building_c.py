...
ensure_directory_exists(file_dir + "/Result_Reference_C")
...
user_bui = {
    ...
    'periods': 1980,                            # Έτος κατασκευής (τυπικά: reference period)
    'latitude': 40.5,                         # Γεωγραφικό πλάτος 
    'longitude': 22.5,                         # Γεωγραφικό μήκος 
    'volume': 450.0,                            # Όγκος κτηρίου (m³)
    'exposed_perimeter': 55.0,                  # Εκτεθειμένη περίμετρος (m)
    'slab_on_ground': 120.0,                    # Εμβαδόν δαπέδου σε επαφή με το έδαφος (m²)
    'wall_thickness': 0.3,                      # Πάχος τοιχοποιίας (m)
    'coldest_month': 1,                         # Ψυχρότερος μήνας (Ιανουάριος)
    'a_use': 187.0,                             # Χρήσιμη επιφάνεια (m²)
    'surface_envelope': 620.0,                 # Συνολική εξωτερική επιφάνεια (m²)
    'surface_envelope_model': 620.0,            # Εξωτερική επιφάνεια για μοντελοποίηση (m²)
    ...
    'cooling_setpoint': 26,                     # Θερμοκρασία ρύθμισης ψύξης (°C)
    ...
    'transmittance_U_elements': [0.44, 0.44, 0.44, 0.44, 0.38, 0.38, 2.8],
    'thermal_resistance_R_elements': [2.27, 2.27, 2.27, 2.27, 2.63, 2.63, 0.36],
...
def main(bui_new: dict, weather_type: str, path_weather_file_: str,
    ...
    boiler_eff = 0.75   # Εποχιακός βαθμός απόδοσης λέβητα
    SCOP = 3.2          # Αντλία θερμότητας - θέρμανση
    SEER = 3.0          # Αντλία θερμότητας - ψύξη
    PEF_el = 2.83        # Συντελεστής πρωτογενούς ενέργειας ρεύματος
    PEF_fuel = 1.33     # Συντελεστής ΠΕ για πετρέλαιο/φυσικό αέριο
    ...
    energy_result_path = os.path.join(dir_chart_folder, "energy_converted_reference_C.csv")
...
if __name__ == "__main__":
    main(
        ...
        path_hourly_sim_result=file_dir + "/Result_Reference_C/hourly_sim_reference_C.csv",
        path_annual_sim_result=file_dir + "/Result_Reference_C/annual_sim_reference_C.csv",
        dir_chart_folder=file_dir + "/Result_Reference_C",
