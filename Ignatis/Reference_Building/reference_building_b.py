...
ensure_directory_exists(file_dir + "/Result_Reference_B")
...
user_bui = {
    ...
    'periods': 2005,                            # Έτος κατασκευής (τυπικά: reference period)
    'latitude': 37.98,                          # Γεωγραφικό πλάτος - Αθήνα
    'longitude': 23.72,                         # Γεωγραφικό μήκος - Αθήνα
    'volume': 276.0,                            # Όγκος κτηρίου (m³)
    'exposed_perimeter': 45.0,                  # Εκτεθειμένη περίμετρος (m)
    'slab_on_ground': 115.0,                    # Εμβαδόν δαπέδου σε επαφή με το έδαφος (m²)
    'wall_thickness': 0.3,                      # Πάχος τοιχοποιίας (m)
    'coldest_month': 1,                         # Ψυχρότερος μήνας (Ιανουάριος)
    'a_use': 115.0,                             # Χρήσιμη επιφάνεια (m²)
    'surface_envelope': 410.0,                  # Συνολική εξωτερική επιφάνεια (m²)
    'surface_envelope_model': 410.0,            # Εξωτερική επιφάνεια για μοντελοποίηση (m²)
    ...
    'transmittance_U_elements': [0.5, 0.5, 0.5, 0.5, 1.0, 0.4, 3.0],  # U-values κατά ΚΕΝΑΚ Ζώνης Β
    'thermal_resistance_R_elements': [2.0, 2.0, 2.0, 2.0, 1.0, 2.5, 0.33],  # 1/U τιμές
...
def main(bui_new: dict, weather_type: str, path_weather_file_: str,
    ...
    boiler_eff = 0.96   # Εποχιακός βαθμός απόδοσης λέβητα
    ...
    energy_result_path = os.path.join(dir_chart_folder, "energy_converted_reference_B.csv")
...
if __name__ == "__main__":
    main(
        ...
        path_hourly_sim_result=file_dir + "/Result_Reference_B/hourly_sim_reference_B.csv",
        path_annual_sim_result=file_dir + "/Result_Reference_B/annual_sim_reference_B.csv",
        dir_chart_folder=file_dir + "/Result_Reference_B",
