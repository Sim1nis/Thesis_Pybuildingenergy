...
result_dir = os.path.join(file_dir, "Result_Tabula_C")
ensure_directory_exists(result_dir)

# === Αρχείο καιρού για Ζώνη Γ ===
# === Ορισμός σχετικής διαδρομής για το custom αρχείο καιρού (EPW) ===
weather_file_path = os.path.join(file_dir, "..", "weather", "GRC_Athens_modified_for_kenak.epw") #Θέλουμε αρχείο καιρού για Ζώνη Γ
weather_file_path = os.path.abspath(weather_file_path)  # Απόλυτο path για το EPW αρχείο

# === Παράμετροι κτηρίου TABULA (Ζώνη Γ) ===
user_bui = {
    ...
    'transmittance_U_elements': [2.2, 2.2, 3.4, 3.1, 3.1, 0.63, 4.5, 4.5],
    'thermal_resistance_R_elements': [1/2.2, 1/2.2, 1/3.4, 1/3.1, 1/3.1, 1/0.63, 1/4.5, 1/4.5],
...
def main(bui_new, weather_type, path_weather_file_,
    ...
    print(f"Ολοκληρώθηκε:", path_annual_sim_result)
    ...
    df_energy.to_csv(os.path.join(dir_chart_folder, "energy_converted_tabula_c.csv"), index=False)
    print("Τελικά αποτελέσματα αποθηκεύτηκαν.")
...
if __name__ == "__main__":
    main(
        ...
        path_hourly_sim_result=os.path.join(result_dir, "hourly_sim_tabula_c.csv"),
        path_annual_sim_result=os.path.join(result_dir, "annual_sim_tabula_c.csv"),
        dir_chart_folder=result_dir,
        name_report="tabula_c_building_report"
    )
